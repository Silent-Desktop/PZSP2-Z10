...
class TestGeneMutation:
    ...
    def test_elitism_preservation(self, mutator, tagged_population):
        ...
        original_elite_val = tagged_population.path_edge_bandwidth_usage[
            0, 0, 1, 0, 1
        ].item()
        new_pop = aggressive_mutator.mutate(
            tagged_population, elite_size, tagged_population.neigh_matrix
        )

        # Elite (Index 0) preserved
        assert (
            new_pop.path_edge_bandwidth_usage[0, 0, 1, 0, 1]
            == original_elite_val
        )

        # Non-Elite (Index 1) mutated
        new_non_elite_val = new_pop.path_edge_bandwidth_usage[
            1, 0, 1, 0, 1
        ].item()
        assert (
            new_non_elite_val != original_elite_val + 1.0
        )  # Original was 101.0

    @patch("net_opt.core.mutations.gene_mutation.Bernoulli")
    def test_mutation_logic_deterministic(
        self, MockBernoulli, mutator, tagged_population
    ):
        """
        ...
        def poisson_side_effect(input_tensor):
            return torch.full_like(input_tensor, 999.0)

        with patch(
            "net_opt.core.mutations.gene_mutation.torch.poisson",
            side_effect=poisson_side_effect,
        ):
            # Patch limits to 10000.0 to prevent clamping interfering with the check
            with patch(
                "net_opt.core.population.Population.edge_size_limits",
                new_callable=PropertyMock,
            ) as mock_limits:
                mock_limits.return_value = torch.full_like(
                    tagged_population.neigh_matrix, 10000.0, dtype=torch.float
                )

                new_pop = mutator.mutate(
                    tagged_population,
                    elite_size,
                    tagged_population.neigh_matrix,
                )
    ...
    def test_clamping_logic(self, mutator, tagged_population):
        ...
        with patch(
            ...
            assert val <= tagged_population.regular_bandwidth
