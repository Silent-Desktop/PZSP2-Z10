...
class TestUniformCrossover:
    ...
    def test_elitism_preservation(self, crossover, labeled_population):
        ...
        elite_size = 1
        original_elite = labeled_population.path_edge_bandwidth_usage[
            0
        ].clone()
        crossover.path_edge_bandwidth_usage_cross_proba = 1.0

        new_pop = crossover.mutate(
            labeled_population, elite_size, labeled_population.neigh_matrix
        )

        assert torch.equal(
            new_pop.path_edge_bandwidth_usage[0], original_elite
        )

    @patch("net_opt.core.mutations.uniform_crossover.torch.randperm")
    @patch("net_opt.core.mutations.uniform_crossover.Bernoulli")
    def test_crossover_mechanics(
        self, MockBernoulli, MockRandperm, crossover, labeled_population
    ):
        """
