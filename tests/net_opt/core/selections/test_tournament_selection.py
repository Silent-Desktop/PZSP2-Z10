...
class TestTournamentSelection:
    ...
    def test_elitism_preservation(self, selector, tagged_population):
        ...
        new_pop = selector.get_next_generation(tagged_population, penalties, elite_size)
    ...
    def test_tournament_logic_deterministic(self, mock_randint, selector, tagged_population):
        """
    ...
    def test_output_tensor_shapes(self, selector, tagged_population):
        ...
        """
        penalties = torch.zeros(4)
        new_pop = selector.get_next_generation(tagged_population, penalties, elite_size=1)
        
        assert new_pop.path_edge_bandwidth_usage.shape == tagged_population.path_edge_bandwidth_usage.shape
        assert new_pop.path_transponder_assignment.shape == tagged_population.path_transponder_assignment.shape
        assert new_pop.encrypted_neigh_matrix.shape == tagged_population.encrypted_neigh_matrix.shape