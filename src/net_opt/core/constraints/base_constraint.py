...
class Constraint(ABC, BaseModel):
    ...
    def _mean_similarity_scores(
        self, scores: Float[Tensor, "P ..."]
    ) -> Float[Tensor, "P"]:
        return torch.mean(scores.flatten(start_dim=1), dim=1)

    @jaxtyped(typechecker=beartype)
    def _calculate_similarity_scores_all(
        self,
        a: Float[Tensor, "P ..."],
        b: Float[Tensor, "P ..."],
        diff_transform: Callable[[Tensor], Tensor] = torch.abs,
    ) -> Float[Tensor, "P ..."]:
    ...
    @jaxtyped(typechecker=beartype)
    def _calculate_similarity_scores_from_sums(
        self,
        a: Float[Tensor, "P ..."],
        b: Float[Tensor, "P ..."],
        diff_transform: Callable[[Tensor], Tensor] = torch.abs,
    ) -> Float[Tensor, "P"]:
        # this one is useful for hard constraints that use relu
        # (demand, bandwidth limit)
        diff = diff_transform(a - b)
        goal = torch.max(a, b)
